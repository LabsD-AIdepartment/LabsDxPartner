...
This decision supersedes prior mandatory social-provider requirements. It does not change earnings, source authority, payment rules or the approved portal layout. Runtime uses the credential adapter and guarded invitation/recovery HTTP, with identity disabled by default. Public username login, invitation setup and reset forms are implemented as local candidates. Authenticated partner leaves still use the deny-only gate; the complete browser journey remains pending. See the implementation receipts for credential adapter, invitation activation and password recovery; this design document is not release acceptance.

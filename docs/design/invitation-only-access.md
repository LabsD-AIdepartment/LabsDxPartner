...
5. Successful redemption activates the preapproved membership, then ordinary credential login starts the authenticated session after the registration transaction commits. Routine activation does not require staff to approve the same deal twice.
...
This decision supersedes prior mandatory social-provider requirements. It does not change earnings, source authority, payment rules or the approved portal layout. Runtime uses the credential adapter and guarded invitation/recovery HTTP, with identity disabled by default. Public username login, invitation setup and reset forms are implemented as local candidates. Authenticated partner leaves now resolve maintained sessions/current memberships and compose the existing feature UI; financial transports remain explicitly unavailable until a real read adapter is connected. The complete HTTPS/browser journey remains pending. See docs/implementation/partner-session-application.md. See the implementation receipts for credential adapter, invitation activation and password recovery; this design document is not release acceptance.

...
- Username normalizes to lowercase after trimming and permits 3–30 Thai/English characters, Thai/ASCII digits, dot and underscore. The shared contract and database constraint (0010) use the same allowed ranges; spaces, invisible separators and currency symbols are excluded. Existing ASCII handles and uniqueness are preserved. Invitation validation identifies and focuses the invalid field. Passwords are never trimmed; new-password contract is8–128 characters (D035; no character-class requirements, shared `passwordPolicy`). Native password hashing and verification are retained.

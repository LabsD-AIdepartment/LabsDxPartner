...
- Username normalizes to lowercase after trimming, permits3–30 ASCII letters/digits/dot/underscore and has a database uniqueness/format constraint. Passwords are never trimmed; new-password contract is8–128 characters (D035; no character-class requirements, shared `passwordPolicy`). Native password hashing and verification are retained.

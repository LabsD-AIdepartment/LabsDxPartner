...
export const transactionDatabase = (tx: TransactionSql) =>
  drizzleAdapter(
    ...
    drizzle(async (query, params, method) => ({
      rows:
        method === 'all' ? await tx.unsafe(query, params).values() : await tx.unsafe(query, params),
    })),
    { provider: 'pg', schema: authSchema, transaction: false },
  );
export const transactionIdentity = (config: IdentityConfig) => (tx: TransactionSql) =>
  createIdentity(config, transactionDatabase(tx));

...
export function createOverviewHttp(access: ReturnType<typeof createPartnerAccess>) {
  ...
  return async (request: Request) => {
    ...
    try {
      ...
      const status = error instanceof AccessFailure
        ? error.code === 'unauthenticated' ? 401 : error.code === 'invalid_input' ? 400 : error.code === 'conflict' ? 409 : 403
        : 503;
      return Response.json({ code: status === 409 ? 'CHANGED' : status === 503 ? 'UNAVAILABLE' : 'ACCESS_DENIED' }, { status, headers });
    }
  };
}
